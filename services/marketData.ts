...
import { Candle, MarketDataError, Quote, Timeframe, TickerInfo } from '@/types/market'
...
function polygonMultiplierAndSpan(tf: Timeframe): { multiplier: number; timespan: string } {
  switch (tf) {
...
async function polygonGetCandles(
  ...
  return json.results.map((r: { t: number; o: number; h: number; l: number; c: number; v: number }) => ({
...
async function polygonSearchTickers(query: string): Promise<TickerInfo[]> {
  ...
  const json = await res.json()
  return (json.results ?? []).map((r: { ticker: string; name: string; primary_exchange: string; type: string }) => ({
    symbol: r.ticker,
    name: r.name,
...
async function twelvedataGetCandles(
  ...
  return json.values.map((r: { datetime: string; open: string; high: string; low: string; close: string; volume: string }) => ({
...
async function twelvedataGetQuote(symbol: string): Promise<Quote> {
  ...
  const json = await res.json()
  if (json.status === 'error') throw new Error(`Twelve Data quote: ${json.message}`)
...
async function twelvedataSearchTickers(query: string): Promise<TickerInfo[]> {
  ...
  const json = await res.json()
  return (json.data ?? []).map((r: { symbol: string; instrument_name: string; exchange: string; instrument_type: string }) => ({
    symbol: r.symbol,
